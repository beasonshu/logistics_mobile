...
public class UpdateManager {
	...
	/* 下载包安装路径 */
	private static final String savePath = Environment.getExternalStorageDirectory().getPath()+File.separator+"update_edition"+File.separator;

	private static final String saveFileName = savePath + "移动车载_司机端.apk";
	...
	private void showNoticeDialog() {
		...
		builder.setPositiveButton("下载", new OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				dialog.dismiss();
				SQLTransaction.getInstance().delteLocalDB(MyApplications.getInstance());
				showDownloadDialog();
			}
		});
		...
		if(CommSet.checkNet(mContext)){
			...
			builder.setNegativeButton("取消", new OnClickListener() {
				...
				public void onClick(DialogInterface dialog, int which) {
					...
					mContext.stopService(new Intent(mContext,MainLogicService.class));
			...
			Toast.makeText(mContext, "请检查网络,您当前在无网络状态下不提供版本更新!", 1).show();
		}
